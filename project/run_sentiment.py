...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        """
        # TODO: Implement for Task 4.5.
        reshaped_emb = embeddings.permute(0, 2, 1)

        out1 = minitorch.nn.max(
            self.conv1(reshaped_emb).relu(),
            2
        )
        out2 = minitorch.nn.max(
            self.conv2(reshaped_emb).relu(),
            2
        )
        out3 = minitorch.nn.max(
            self.conv3(reshaped_emb).relu(),
            2
        )

        combined_out = out1 + out2 + out3

        flattened_out = self.linear(
            combined_out.view(
                combined_out.size // self.feature_map_size,
                self.feature_map_size
            )
        )

        final_out = minitorch.nn.dropout(
            flattened_out,
            self.dropout,
            not self.training
        )
        return final_out.sigmoid().view(final_out.shape[0])
